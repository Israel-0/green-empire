    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-grow-darker via-[#0a120a] to-grow-darker relative overflow-hidden">
      ...
      >
        ...
        >
          ...
          <p className="text-grow-muted text-sm mt-2">Crea tu cuenta y empieza a cultivar</p>
        </motion.div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
            <button type="submit" disabled={loading} className="btn-primary w-full py-3 text-base font-semibold tracking-wide">
              {loading ? (
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
          ...
          ¿Ya tienes cuenta?{' '}
          <Link to="/login" className="text-grow-green hover:text-grow-green-bright transition-colors font-medium">
            Inicia sesión
          </Link>
        </motion.p>
      </motion.div>
